...
# ── quick test ────────────────────────────────────────────────────────────────
# Run this file directly to exercise both LLM calls against a live local
# Ollama instance. Requires `ollama serve` running and the model from
# OLLAMA_MODEL (default llama3.2:3b) already pulled.
# Usage: python src/generator.py
# ──────────────────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    print("Running generator test...\n")
    print(f"Ollama base URL: {_OLLAMA_BASE_URL}")
    print(f"Model: {_OLLAMA_MODEL}\n")

    # test 1: query decomposition on a question that bundles two info needs
    question = "How does LoRA compare to full fine-tuning, and how does it relate to attention layers?"
    ...
    # test 2: decomposition on an already-narrow question (should not over-split)
    narrow_question = "What is the embedding dimension used in the paper?"
    print(f"Narrow question: {narrow_question}")
    narrow_result = decompose_query(narrow_question)
    print(f"Decomposed into {len(narrow_result)} sub-quer{'y' if len(narrow_result) == 1 else 'ies'}: {narrow_result}\n")

    # test 3: answer generation with a small hand-built context, simulating
    # what pipeline.py would assemble from retriever.py's output
    system_prompt = (
        "You are a research assistant. Answer ONLY using the provided context. "
        "Cite the source paper for every claim. If the context does not contain "
        "the answer, say so explicitly instead of guessing."
    )
    user_prompt = (
        "Context:\n"
        "[Source: vaswani_2017.pdf, p.3] The Transformer uses multi-head "
        "self-attention, allowing the model to jointly attend to information "
        "from different representation subspaces at different positions.\n\n"
        "Question: What kind of attention does the Transformer use?"
    )
    print("Generating answer from hand-built test context...")
